...
def degree_stats(graph_ref_list, graph_pred_list, is_parallel=True, kernel = "gaussian_tv"):
  ...
  if not kernel == "gaussian_tv":
    print("using gaussian_emd Kernel to compute degree MMD")
...
def spectral_stats(graph_ref_list, graph_pred_list, is_parallel=True, kernel = "gaussian_tv"):
  ''' Compute the distance between the degree distributions of two unordered sets of graphs.
    Args:
      graph_ref_list, graph_target_list: two lists of networkx graphs to be evaluated
    '''
...
def orbit_stats_all(graph_ref_list, graph_pred_list, kernel="gaussian_tv"):
  for G_ in graph_ref_list:
    G_.remove_edges_from(nx.selfloop_edges(G_))
  graph_ref_list = [G_ for G_ in graph_ref_list if not G_ is None]

  for G_ in graph_pred_list:
    G_.remove_edges_from(nx.selfloop_edges(G_))
  graph_pred_list = [G_ for G_ in graph_pred_list if not G_ is None]
