...
class KernelPENodeEncoder(torch.nn.Module):
    ...
    def __init__(self, config, dim_emb=None, expand_x=False):
        ...
        pecfg = getattr(config, f"posenc_{self.kernel_type}")
        ...
        activation = nn.ReLU  # register.act_dict[config.gnn.act]
        if model_type == 'mlp':
            layers = []
            if n_layers == 1:
...
@register_node_encoder('Cycles')
class CyclesNodeEncoder(KernelPENodeEncoder):
    """k-Cycles Encoding node encoder.
    """
    kernel_type = 'Cycles'
