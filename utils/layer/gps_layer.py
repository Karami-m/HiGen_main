...
from utils.layer.bigbird_layer import SingleBigBirdLayer
from utils.layer.gatedgcn_layer import GatedGCNLayer
from utils.layer.gine_conv_layer import GINEConvESLapPE
...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        # MPNNs supporting also edge attributes.
            ...
            self.local_model = pygnn.PNAConv(dim_h, dim_h,
                                             ...
                                             post_layers=1,
                                             divide_input=False)
        elif local_gnn_type in ['CustomGatedGCN', 'CustomGatedGCN2']:
            self.local_model = GatedGCNLayer(dim_h, dim_h,
                                             dropout=dropout,
                                             residual=True,
                                             act=act,
                                             equivstable_pe=equivstable_pe,
                                             aug_model = local_gnn_type == 'CustomGatedGCN2')
        else:
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            if self.local_gnn_type in ['CustomGatedGCN', 'CustomGatedGCN2']:
                es_data = None
                ...
                local_out = self.local_model(
                    Batch(batch=batch,
                          ...
                          edge_index_aug = batch.edge_index_aug if hasattr(batch, 'edge_index_aug') else None,
                          edge_attr_aug = batch.edge_attr_aug if hasattr(batch, 'edge_attr_aug') else None
                          )
                )
                # GatedGCN does residual connection and dropout internally.
                h_local = local_out.x
                batch.edge_attr = local_out.edge_attr
                batch.edge_attr_aug = local_out.get('edge_attr_aug', None)
            else:
        ...
        if self.self_attn is not None:
            ...
            if self.global_model_type == 'Transformer':
                ...
                if attn_bias is None:
                    ...
                    h_attn = self._sa_block(h_dense, attn_bias, None)[mask]
