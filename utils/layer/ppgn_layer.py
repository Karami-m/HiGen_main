# Based on https://github.com/KarolisMart/SPECTRE/blob/main/model/ppgn.py and https://github.com/cvignac/SMP/blob/master/models/ppgn.py
import torch
import torch.nn as nn
import torch.nn.functional as F

from utils.nn import masked_instance_norm2D, masked_layer_norm2D

SLOPE = 0.01


class PowerfulLayer(nn.Module):
    def __init__(self, in_feat: int, out_feat: int, num_layers: int, activation=nn.LeakyReLU(negative_slope=SLOPE),
                 spectral_norm=(lambda x: x)):
        super().__init__()
        self.in_feat = in_feat
        self.out_feat = out_feat
        self.m1 = nn.Sequential(
            *[spectral_norm(nn.Linear(in_feat if i == 0 else out_feat, out_feat)) if i % 2 == 0 else activation for i in
...
class FeatureExtractor(nn.Module):
    def __init__(self, in_features: int, out_features: int, activation=nn.LeakyReLU(negative_slope=SLOPE),
                 spectral_norm=(lambda x: x)):
        super().__init__()
...
class Powerful(nn.Module):
    def __init__(self, num_layers: int, input_features: int, hidden: int, hidden_final: int, dropout_prob: float,
        ...
        for i in range(num_layers):
            self.convs.append(
                PowerfulLayer(hidden, hidden, layers_per_conv, activation=self.activation, spectral_norm=spectral_norm))
        ...
        for i in range(num_layers):
            ...
            self.feature_extractors.append(
                FeatureExtractor(hidden, hidden_final, activation=self.activation, spectral_norm=spectral_norm))
    ...
    def forward(self, A, node_features, mask):
        ...
        for conv, extractor, bn in zip(self.convs, self.feature_extractors, self.bns):
            ...
            if self.normalization == 'none':
                ...
                u = masked_instance_norm2D(u,
                                           mask)  # Didn't test it, but GraphNorm (https://arxiv.org/abs/2009.03294) might work even better
